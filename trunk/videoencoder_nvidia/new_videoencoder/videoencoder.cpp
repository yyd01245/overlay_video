...
void yuv12_copy_to_nv12(char* yuv420,int iwidth,int iheight,  char * nv12_buf){
	int size = iwidth* iheight; 
	int width, height, width2, height2, pitch;
	width = iwidth;
	height = iheight;
	width2 = width/ 2;
	height2 = height / 2;
...
#endif
	memcpy(y_ptr, y_src, size);
	char *u_ptr = nv12_buf+size;
	char *v_ptr = u_ptr + 1;
	char *u_src = yuv420 + size  ;
	char *v_src = yuv420 + size + size / 4;
	...
	char *output_video_sample,int *output_video_sample_size,int x,int y,int w,int h)
{
	...
	memcpy(p_instanse->m_yuv,input_video_sample,input_video_sample_size);
	//if(input_video_sample_size!=0){

	//	yuv12_copy_to_nv12(const_cast<char *>(input_video_sample),p_instanse->width,p_instanse->height,p_instanse->m_yuv);

#if 0
		if(NULL==fpyv12)
			fpyv12 = fopen("testyv12.yuv","wb");
		fwrite(input_video_sample,1,input_video_sample_size,fpyv12);
		printf("input size=%d \n", input_video_sample_size);
	//	yuv12_copy_to_nv12(const_cast<char *>(input_video_sample),p_instanse->width,p_instanse->height,p_instanse->m_yuv);

		if(NULL==fpnv12)
			fpnv12 = fopen("testnv12.yuv","wb");
		fwrite(p_instanse->m_yuv,1,input_video_sample_size,fpnv12);
#endif
	//}
#endif
	*output_video_sample_size =p_instanse->m_pfnv_encoder_encodeframe(p_instanse->nvencoder,const_cast<char *>(input_video_sample),output_video_sample);
