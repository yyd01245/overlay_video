...
int NvEncoder_InitDefaultConfig(EncodeConfig *encCfg)
{
    ...
    encCfg->intraRefreshEnableFlag=1;
   	encCfg->intraRefreshPeriod=0;  //100 I 帧分片
    encCfg->intraRefreshDuration=0; //I 帧分片
	//4
    ...
    encCfg->rcMode= NV_ENC_PARAMS_RC_VBR; //NV_ENC_PARAMS_RC_VBR;//NV_ENC_PARAMS_RC_CBR;
...
int NvEncoder_Initialize(nvEncodeHandle* pHandle,char argc,char**argv)
{
    ...
    //enc->encCfg.intraRefreshPeriod=enc->encCfg.gopsize;//using gopsize instead of IRP for output I frame;
   // enc->encCfg.intraRefreshDuration=1;
...
//		iFlag = 1;
	...
	//	printf("-----begin find I frame spslen =%d \n",*enc->sequenceParamPayload->outSPSPPSPayloadSize);
		//int iposIframe = Find_H264_IFrameHead((unsigned char*)bstream.bitstreamBufferPtr,eslen);
	//	if(iposIframe >= 0)
		if(iFlag)
		{
