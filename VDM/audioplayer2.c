...
//  printf("  latency %8d us\n request length:%zu\n",(int)usec,length);
  ...
  int nr=read(STDIN_FILENO,sampledata,length);
  pa_stream_write(s, sampledata, nr, NULL, 0LL, PA_SEEK_RELATIVE);
  printf("  latency %8d us\n request length:%zu\n read stdin:%d\n",(int)usec,length,nr);
#endif
...
int main(int argc, char *argv[]) {
  ...
  fcntl(STDIN_FILENO,fl|O_NONBLOCK);
#endif
...
//  pthread_t tid;
//  pthread_create(&tid,NULL,aud_recv,NULL);
